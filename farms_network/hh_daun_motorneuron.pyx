...
cdef class HHDaunMotorneuron(Neuron):
    ...
    def __init__(self, n_id, num_inputs, neural_container, **kwargs):
        ...
        (_, self.am1_nap) = neural_container.constants.add_parameter(
            'am1_nap' + self.n_id, kwargs.get('am1_nap', 0.32))
        (_, self.am2_nap) = neural_container.constants.add_parameter(
            'am2_nap' + self.n_id, kwargs.get('am2_nap', -51.90))
        (_, self.am3_nap) = neural_container.constants.add_parameter(
            'am3_nap' + self.n_id, kwargs.get('am3_nap', 0.25))
        (_, self.bm1_nap) = neural_container.constants.add_parameter(
            'bm1_nap' + self.n_id, kwargs.get('bm1_nap', -0.280))
        (_, self.bm2_nap) = neural_container.constants.add_parameter(
            'bm2_nap' + self.n_id, kwargs.get('bm2_nap', -24.90))
        (_, self.bm3_nap) = neural_container.constants.add_parameter(
            'bm3_nap' + self.n_id, kwargs.get('bm3_nap', -0.2))
        (_, self.ah1_nap) = neural_container.constants.add_parameter(
            'ah1_nap' + self.n_id, kwargs.get('ah1_nap', 0.1280))
        (_, self.ah2_nap) = neural_container.constants.add_parameter(
            'ah2_nap' + self.n_id, kwargs.get('ah2_nap', -48.0))
        (_, self.ah3_nap) = neural_container.constants.add_parameter(
            'ah3_nap' + self.n_id, kwargs.get('ah3_nap', 0.0556))
        (_, self.bh1_nap) = neural_container.constants.add_parameter(
            'bh1_nap' + self.n_id, kwargs.get('bh1_nap', 4.0))
        (_, self.bh2_nap) = neural_container.constants.add_parameter(
        ...
        (_, self.am1_k) = neural_container.constants.add_parameter(
            'am1_k' + self.n_id, kwargs.get('am1_k', 0.0160))
        (_, self.am2_k) = neural_container.constants.add_parameter(
            'am2_k' + self.n_id, kwargs.get('am2_k', -29.90))
        (_, self.am3_k) = neural_container.constants.add_parameter(
            'am3_k' + self.n_id, kwargs.get('am3_k', 0.20))
        (_, self.bm1_k) = neural_container.constants.add_parameter(
            'bm1_k' + self.n_id, kwargs.get('bm1_k', 0.250))
        (_, self.bm2_k) = neural_container.constants.add_parameter(
        ...
        (_, self.gamma_q) = neural_container.constants.add_parameter(
            'gamma_q' + self.n_id, kwargs.get('gamma_q', -0.6))
        (_, self.r_q) = neural_container.constants.add_parameter(
        ...
        (_, self.v_hs) = neural_container.constants.add_parameter(
        ...
        self.h_na_dot = neural_container.dstates.add_parameter(
            'h_na_dot_' + self.n_id, 0.0)[0]
        self.m_k_dot = neural_container.dstates.add_parameter(
