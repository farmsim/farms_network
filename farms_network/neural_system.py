...
class NeuralSystem(NetworkXModel):
    ...
    def rk5(self, time, state, func, step_size=1e-3):
        ...
        K1 = np.asarray(func(time, state))
        K2 = np.asarray(func(time + step_size/4.0, state + (step_size/4.0 * K1)))
        K3 = np.asarray(func(time + step_size/4.0, state + (step_size/8.0)*(K1 + K2)))
        K4 = np.asarray(func(time + step_size/2.0, state - (step_size/2.0 * K2) + (step_size * K3)))
        K5 = np.asarray(func(time + 3*step_size/4.0, state + (step_size/16.0)*(3*K1 + 9*K4)))
        K6 = np.asarray(func(time + step_size, state + (step_size/7.0)*(-3*K1 + 2*K2 + 12*K3 + -12*K4 + 8*K5)))
        new_state = np.asarray(state) + (7/90*K1 + 32/90*K3 + 12/90*K4 + 32/90*K5 + 7/90*K6)*(step_size)
        return new_state
