...
\begin{{document}}
\definecolor{{flex}}{{RGB}}{{127,201,127}}
\definecolor{{inf}}{{RGB}}{{190,174,212}}
\definecolor{{ext}}{{RGB}}{{253,192,134}}
...
def main():
    ...
    rhythm1 = RhythmGenerator(name="")
    rhythm2 = RhythmGenerator(name="2", anchor_x=5.0, anchor_y=0.0)
    rhythm3 = RhythmGenerator(name="3", anchor_x=0.0, anchor_y=5.0)
    rhythm4 = RhythmGenerator(name="4", anchor_x=5.0, anchor_y=5.0)
    rhythm = nx.compose_all(
        [rhythm1, rhythm2, rhythm3, rhythm4]
    )
    rhythm.nodes["-RG-F"]["style"] = "my-node"
    print(rhythm.nodes["-RG-F"])
    node_options = {
        "-RG-F": "my-node, fill=flex!40",
        "-RG-E": "my-node, fill=ext!40",
        "-In-E": "my-node, fill=inf!40",
        "-In-F": "my-node, fill=inf!40",
    }
    rhythm.nodes["-RG-E"]["style"] = "line width=3,draw"
    nx.write_graphml(rhythm, "./config/auto_zhang_2022.graphml")
    nx.write_latex(
        rhythm,
        "zhang2022_figure.tex",
        pos={name: (node['x'], node['y']) for name, node in rhythm.nodes.items()},
        as_document=True,
        caption="A path graph",
        latex_label="fig1",
        node_options=node_options,
        default_node_options="my-node",
        node_label={"-RG-F": "F", "-RG-E": "E", "-In-F": "In", "-In-E": "In"},
        default_edge_options="[color=black, thick, -{Latex[scale=1.0]}, bend left, looseness=0.75]",
        document_wrapper=_DOC_WRAPPER_TIKZ,
    )
    latex_code = nx.to_latex(rhythm)  # a string rather than a file
